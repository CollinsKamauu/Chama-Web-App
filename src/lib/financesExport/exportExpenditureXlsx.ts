...
export async function exportExpenditureXlsx(
  rows: ExpenditureRow[],
  periodLabel: string,
  exportedAt: Date,
): Promise<void> {
  ...
  const headers = ['Transaction ID', 'Date', 'Name', 'Phone Number', 'Type', 'Amount']
  headers.forEach((h, i) => {
  ...
  for (const row of rows) {
    ...
    ws.getCell(r, 5).value = row.type
    ws.getCell(r, 6).value = row.amount
    ...
    for (let c = 1; c <= COL_COUNT; c += 1) {
      ...
      cell.alignment = {
        vertical: 'middle',
        ...(c === 6 ? { horizontal: 'right' } : { horizontal: 'left' }),
      }
    }
  ...
  ws.getCell(r, 6).numFmt = '#,##0'
  ws.getCell(r, 6).font = { bold: true, name: 'Arial', size: 11 }
  ws.getCell(r, 6).alignment = { horizontal: 'right', vertical: 'middle' }
  ws.getRow(r).height = 26

  ws.columns = [{ width: 22 }, { width: 14 }, { width: 26 }, { width: 22 }, { width: 18 }, { width: 14 }]
  ...
  downloadBlob(
    ...
    `milestone-fraternity-expenditures-${formatExportStamp(exportedAt)}.xlsx`,
  )
}
